...
You will need a **JavaMail** session to send notifications; instructions on setting up a `JavaMail` session can be found xref:Technical Documentation/Payara Server Documentation/General Administration/javamail.adoc[in the core documentation of the JavaMail API].
