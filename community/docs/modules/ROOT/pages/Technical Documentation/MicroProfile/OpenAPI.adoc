...
==== Deployed endpoints
In case we have more than one application deployed, a merge is done in openapi document. The resultant document has all applications and paths. In order to make the document more legible, the attribute `endpoints` was added as a Payara Platform-specific extension which lists all deployed endpoints grouped by application context root.
