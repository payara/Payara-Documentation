...
For more information about clusters, see the xref:docs:ha-administration-guide:overview.adoc[Payara Server High Availability Administration Guide].
...
. Ensure that the server is running. Remote subcommands require a running server.
. Create a transport by using the xref:docs:reference-manual:create-transport.adoc[`create-transport`]
