...
. List batch jobs by using the xref:docs:administration-guide:jdbc.adoc[list-batch-jobs]
subcommand.
...
. Ensure that the server is running. +
Remote subcommands require a running server.
. List batch job executions by using the xref:docs:administration-guide:jdbc.adoc[list-batch-job-executions] subcommand.
