...
to view log information in the Payara Server environment.
...
Logging is the process by which Payara Server
...
Payara Server Open Source Edition log records are captured in the
...
In an Payara Server Open Source Edition domain, log files have the
...
Payara Server writes log records in either the
...
By default, when a log file grows to 2 MB, Payara Server renames 
(rotates) the file to incorporate a timestamp and
...
Payara Server provides a logger for each of its
...
see xref:docs:administration-guide:logging.adoc#to-list-log-levels[To List Log Levels]. For information about how to
display a list of loggers and logger details, see xref:docs:administration-guide:logging.adoc#to-list-loggers[To List
Loggers].

[[logging-targets]]
=== Logging Targets

Each instance in an Payara Server domain has a
...
Payara Server allows you target specific log
...
* xref:docs:administration-guide:logging.adoc#changing-the-name-and-location-of-logging-service-files[Changing the Name and Location of Logging Service Files]
* xref:docs:administration-guide:logging.adoc#setting-log-levels[Setting Log Levels]
* xref:docs:administration-guide:logging.adoc#setting-the-log-file-format[Setting the Log File Format]
* xref:docs:administration-guide:logging.adoc#setting-log-file-rotation[Setting Log File Rotation]
* xref:docs:administration-guide:logging.adoc#adding-a-custom-logging-handler[Adding a Custom Logging Handler]
...
. Use the xref:docs:reference-manual:list-log-attributes.adoc#GSRFM00182[`list-log-attributes`] subcommand in remote
...
. Use the xref:docs:reference-manual:set-log-attributes.adoc#GSRFM00227[`set-log-attributes`] subcommand in remote
...
other Payara Server  logging properties files on
...
restart Payara Server for changes to take effect.
...
[[to-list-log-levels]]
==== To List Log Levels
Payara Server provides the means to list all
...
xref:docs:reference-manual:list-log-levels.adoc#GSRFM00183[`list-log-levels`] subcommand.
...
xref:docs:reference-manual:set-log-levels.adoc#GSRFM00228[`set-log-levels`] subcommand, specifying the log level
...
. List the existing module loggers and their log levels by using the
xref:docs:reference-manual:list-log-levels.adoc#GSRFM00183[`list-log-levels`] subcommand.
. Set the log level for a module by using the
xref:docs:reference-manual/set-log-levels.adoc#GSRFM00228[`set-log-levels`] subcommand. +
...
formatter used by Payara Server to format log
...
the same. For information about log formats, see xref:docs:administration-guide:logging.adoc#log-records[Log
Records].
...
xref:docs:reference-manual:redeploy.adoc#GSRFM00217[`set-log-file-format`] subcommand.
. To apply your change, restart Payara Server.
...
. Exclude fields by using the link:../reference-manual/set-log-attributes.html#GSRFM00227[`set-log-attributes`]
subcommand, specifying the following attribute and the fields to
exclude: `com.sun.enterprise.server.logging.GFFileHandler.excludeFields=fields`
where `fields` is a comma-separated list of the name-value fields to
exclude from the log file.
. To apply your change, restart Payara Server.

[[exampleto-exclude-fields-in-log-records]]
...
xref:docs:reference-manual:set-log-attributes.adoc#GSRFM00227[`set-log-attributes`] subcommand, specifying the
...
As explained in xref:docs:administration-guide:logging.adoc#log-files[Log Files],Payara Server
 by default rotates log files when they reach 2 MB in size.
...
By default, Payara Server does not limit the
...
xref:docs:reference-manual:set-log-attributes.adic#GSRFM00227[`set-log-attributes`] subcommand, specifying the
...
xref:docs:reference-manualset-log-attributes.html#GSRFM00227[`set-log-attributes`] subcommand, specifying the
...
Payara Server for changes to take effect.

. Ensure that the target server or cluster is running.
. Rotate log files by using the xref:docs:reference-manual:rotate-log.adoc#GSRFM00224[`rotate-log`]
subcommand.
...
By default, Payara Server log records are
captured in a server log file using the format described in
xref:docs:administration-guide:logging.adoc#viewing-log-records[Log Records]. 
However, you may find that you want to log
...
custom log handler to the Payara Server logging
service.
...
In Payara Server, the best approach to developing
a custom handler is to define a Hundred-Kilobyte Kernel (HK2) component
that implements the handler contract. Payara Server registers this 
handler automatically because it is an HK2
...
. Ensure that the target server or cluster is running. Remote subcommands require a running server.
. Use the xref:docs:reference-manual:set-log-attributes.adoc#GSRFM00227[`set-log-attributes`] subcommand to add the
handler to the `handlers` attribute. The default target of this subcommand is the DAS. Optionally you can
target a configuration, server, instance, or cluster.
. To apply your changes, restart Payara Server. +
See xref:docs:administration-guiide:domains.adoc#to-restart-a-domain[To Restart a Domain].
...
Payara Server also allows you to collect log
...
. Ensure that the target server or cluster is running. Remote subcommands require a running server.
. Use the xref:docs:reference-manualL:collect-log-files.adoc#GSRFM00007[`collect-log-files`] subcommand to create
the ZIP archive. The default location in which the ZIP archive is created is the
...
. Ensure that the target server or cluster is running. Remote commands require a running server.
. List loggers by using the xref:docs:reference-manualredeploy.adoc#list-loggers[`list-loggers`]
subcommand.
