...
To display the available subcommands, use the xref:docs:reference-manual:list-commands.adpc[`list-commands`] subcommand. Local subcommands are displayed before remote subcommands. If the server is not running, only local subcommands are displayed.
...
NOTE: Each `POST` method and `DELETE` method that a REST resource supports has an equivalent `asadmin` subcommand. The parameters of a `POST` method or a `DELETE` method correspond to the options of the method's equivalent `asadmin` subcommand. For information about the options of `asadmin` subcommand, see the xref:docs:administration-guide:asadmin-subcommands.adoc[Payara Server Open Source Edition Reference Manual].
