...
* To use the local file system, first disable availability. Select the _Availability Service_ component under the relevant configuration in the  Administration Console. Uncheck the _Availability Service_ box. Then  select the EJB Container component and edit the Session Store Location value. The default is `${DOMAIN_DIR}/session-store`.
