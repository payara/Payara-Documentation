...
The solution is to remove the marker file on the DAS, restart the DAS and the clusters or deployment groups, and then redeploy any applications that rely on the offending EJB timer. The marker file is located on the DAS in `${domain-dir}/generated/ejb/ejb-timer-service-app`.
