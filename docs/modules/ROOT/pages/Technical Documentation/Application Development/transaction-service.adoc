...
For more information about the Jakarta Transactions API (JTA) and Java Transaction Service (JTS), see "xref:Technical Documentation/Payara Server Documentation/General Administration/transactions.adoc#administering-transactions[Administering Transactions]" in
Payara Server General Administration section and in the https://jakarta.ee/specifications/transactions[Jakarta Transactions specification].
...
The default setting is `false`. For more information about non-component callers, see xref:Technical Documentation/application-development-guide:jdbc.adoc#allowing-non-component-callers[Allowing Non-Component Callers].
