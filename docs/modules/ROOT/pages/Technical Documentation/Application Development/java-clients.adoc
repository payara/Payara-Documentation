...
JMS resources are supported only in the Full Profile, not in the Web Profile. See xref:Technical Documentation/Application Development/jms.adoc#using-jakarta-messaging[Using the Java Message Service].
...
For information about debugging application clients, see xref:Technical Documentation/Application Development/debugging-apps.adoc#application-client-debugging[Application Client Debugging].
...
The ACC integrates with Payara Server's authentication system. It also supports SSL (Secure Socket Layer)/IIOP if configured and when necessary; see xref:Technical Documentation/Application Development/java-clients.adoc#using-rmiiiop-over-ssl[Using RMI/IIOP Over SSL].
...
Application clients can use a programmatic login mechanism by xref:Technical Documentation/Application Development/securing-apps.adoc#programmatic-login-using-the-programmaticlogin-class[using the `ProgrammaticLogin` Class].
...
For more information about naming and lookups, see xref:Technical Documentation/Application Development/jndi.adoc#accessing-the-naming-context[Accessing the Naming Context]. +
...
The user has to log in to the ACC using either the `sun-acc.xml` mechanism or the xref:Technical Documentation/Application Development/securing-apps.adoc#programmatic-login-using-the-programmaticlogin-class[Programmatic Login Using the `ProgrammaticLogin` Class] mechanism.
