...
You must ensure that the `server.policy` file is appropriately set up, or a lifecycle module trying to perform a `System.exec()` might cause a security access violation. For details, see xref:Technical Documentation/application-development-guide:securing-apps.adoc#the-server.policy-file[The *server.policy* File].
