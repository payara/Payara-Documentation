...
For more information about `server.policy`, see xref:Technical Documentation/Application Development/securing-apps.adoc#the-server.policy-file[The `server.policy` File].
...
For information about setting up load balancing for message-driven beans, see xref:Technical Documentation/Application Development/jms.adoc#load-balanced-message-inflow[Load-Balanced Message Inflow].
...
* The EJB Timer Service needs to use shared storage --an external database-- or xref:Technical Documentation/Payara Server Documentation/High Availability/domain-data-grid.adoc[the DataGrid].
