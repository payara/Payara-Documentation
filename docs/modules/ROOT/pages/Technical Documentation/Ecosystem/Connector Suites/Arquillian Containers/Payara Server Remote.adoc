...
The context root that will be used to run the tests is also retrieved automatically from the DAS server. If the deployed application does have a `sun-web.xml`, `payara-web.xml` deployment descriptor, the container will use the name of your deployment without the extension as context root. +
