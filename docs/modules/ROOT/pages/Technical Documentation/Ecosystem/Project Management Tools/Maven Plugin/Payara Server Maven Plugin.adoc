...
|PAYARA_AI_AGENT
|false
|Enables the AI Agent. See <<ai-agent>> for full configuration.
...
The AI Agent is an experimental feature that brings intelligent automation and observability to Jakarta EE application development directly from the Maven terminal.
Enable it by setting `PAYARA_AI_AGENT=true` (or `payara.ai.agent=true`) and supplying an AI provider API key.

Once started, a highlighted `payara` prompt appears in the terminal. Type any natural language query to interact with the running server.

For full configuration reference — model providers, server connection properties, asadmin auto-execution policy, and the RAG documentation index — see xref:/Technical Documentation/Ecosystem/Payara AI Agent.adoc[Payara AI Agent].
