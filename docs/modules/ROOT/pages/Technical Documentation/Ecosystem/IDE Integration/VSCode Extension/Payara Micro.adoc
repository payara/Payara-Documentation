...
* When you fill the required project details, the IDE creates the following projects from the Payara Micro maven archetype. You may add the generated project to the current workspace or open in the new window:
...
* Now to simply run the Payara Micro application, go to the explorer sidebar (or Payara activity bar), expand Payara Micro Instances, right-click on the listed Payara Micro instance and select *Start*:
...
* Payara Micro Application supports auto-deploy feature which redeploys the new application on every save action. To manually redeploy the application, right-click on the listed Payara Micro instance and select *Redeploy*: 
