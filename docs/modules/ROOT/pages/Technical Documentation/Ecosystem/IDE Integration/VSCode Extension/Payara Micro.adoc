...
The extension also allows you to use the Hot Deploy and AutoDeploy features for a better developer experience. Any Maven project appears in the *Micro Projects* panel — no plugin declaration in `pom.xml` is required to run with the full plugin coordinates.
...
Dev mode enables automatic hot deployment of changes without restarting the Payara Micro instance. Click the *Start (Dev Mode)* button (lightning bolt icon) in the *Micro Projects* panel to start in dev mode.

[[debug-mode]]
== Debug Mode

The extension supports attaching the VS Code Java debugger to a running Payara Micro instance. Use *Start in Debug Mode* or *Start (Dev Mode) in Debug* from the toolbar or right-click menu.

The default debug port is *5005*. On the first debug launch, the extension automatically creates a `payara-micro` entry in `.vscode/launch.json`. To use a different port, edit that entry:
