...
* xref:Technical Documentation/Ecosystem/IDE Integration/VSCode Extension/Payara Server Maven Plugin.adoc[Payara Server Maven Plugin] - The Payara VS Code extension provides support for the `payara-server-maven-plugin`, allowing developers to start, stop, and run Payara Server in dev mode directly from VS Code without requiring plugin declaration in `pom.xml`.
