...
The setup of a Docker node is more involved that other types of nodes, so it is recommended reading about the requirements and conditions for this command to work properly in xref:Technical Documentation/Payara Server Documentation/High Availability/nodes.adoc#creating-a-docker-node[corresponding section] in Payara Server's High Availability section.
