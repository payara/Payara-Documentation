...
[--source={domain|config|server|application|module}]
----
...
+
Must be one of: `domain`, `config`, `server`, `application`, `module`
`--target`::
...
This example gets the ordinal value for the source 'domain'.

[source,shell]
----
asadmin> get-config-ordinal --source=domain
----
