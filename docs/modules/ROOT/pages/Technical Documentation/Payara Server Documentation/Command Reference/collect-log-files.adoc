...
the domain administration server (DAS), the specified deployment group, or the specified Payara Server instance and creates a single ZIP archive of the log files.
...
`deploymenmt-group-name`;;
    The log files will be collected for the specified deployment group.
...
[[example-2]]
*Example 2 Collecting Log Files for a Deployment Group*

This example generates a ZIP archive from the log files for a deployment group named `cluster1` and the two server instances running in the group.
...
[[example-3]]
*Example 3 Collecting Log Files in a Directory Other Than the Default for a Deployment Group*

This example generates a ZIP archive from the log files for a deployment group named `cluster1` and its two server instances, and saves the archive in a directory named `/space/output`.
