...
`--installdir`::
  The absolute path to the parent of the base installation directory where the Payara Server software is installed on each host, for example, `C:\payara7`. +
...
The software is uninstalled even if a user-defined node resides on the host. The entire content of the `C:\payara7` directory is removed.

[source,shell]
----
asadmin> uninstall-node-dcom --force --installdir C:\payara7 xkyd.example.com
Command uninstall-node-dcom executed successfully.
----
