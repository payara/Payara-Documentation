...
| `setup-local-dcom`
| Sets up the Distributed Component Object Model (DCOM) remote protocol on the host where the subcommand is run. The DCOM remote protocol is available only on Windows systems. This command is deprecated and no longer supported in Azul Payara Server 5.89.0 and 6.40.0, and Payara Platform Community Edition 7 onwards..
