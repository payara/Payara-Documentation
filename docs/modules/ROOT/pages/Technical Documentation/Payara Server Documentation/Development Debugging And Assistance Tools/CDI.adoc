...
This mode also provides a <<CDI Probe>>, which allows the inspection and monitoring of CDI beans of deployed applications at runtime.
...
* Through the application defining a context parameter in web.xml (enabled by default)
...
Once CDI Development Mode is enabled for an application, the *Edit Application* page in the Admin Console provides a link to open the CDI Probe web application for quick access.
