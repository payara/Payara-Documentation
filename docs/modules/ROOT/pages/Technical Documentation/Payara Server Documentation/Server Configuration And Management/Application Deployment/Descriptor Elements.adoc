...
With this property, you can configure the time to live (TTL) in milliseconds of the virtual server cache for serving files. This element is set up as a `property` element with an integer (String) value attribute like this:
