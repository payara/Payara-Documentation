...
* `managed-executor-service` (configure a new ManagedExecutorService)
* `managed-scheduled-executor-service` (configure a new ManagedScheduledExecutorService)
* `managed-thread-factory` (configure a new ManagedThreadFactory)

Configuration of concurrency objects is demonstrated in https://github.com/payara/Payara/blob/master/appserver/tests/payara-samples/samples/resources/mes-in-payara-resources-example/src/main/webapp/WEB-INF/payara-resources.xml[Payara Samples - ManagedExecutorService in payara-resources.xml].
