...
With the `proxyDefinition` parameter of the `OpenIdAuthenticationDefinition`, you can specify details regarding proxy mapping within the OpenID Connect client configuration. It contains attributes to specify the `hostName` and `port` of the proxied server. This addition facilitates the configuration of proxy mapping for requests traversing through reverse web proxies.
...
When using this standalone connector, import classes with the with the `fish.payara.security.connectors` package instead of the `fish.payara.security`.

[[download-the-standalone-openid-connect-connector]]
=== Download the standalone OpenID Connect connector

You can access the latest version in the https://nexus.payara.fish/service/rest/repository/browse/payara-artifacts/fish/payara/security/connectors/openid-standalone/[Payara Artifacts Maven Repository].
...
Dynamic configuration is possible using Expression Language to specify the properties in the configuration annotations as described in xref:/Technical Documentation/Public API/OpenID Connect Support.adoc#el-support[Payara Documentation].
