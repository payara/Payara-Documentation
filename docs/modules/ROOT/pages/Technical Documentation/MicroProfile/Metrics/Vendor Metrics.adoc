...
xref:Technical Documentation/Payara Server Documentation/Logging and Monitoring/Monitoring Service/MBeans.adoc[JMX MBeans] can be exposed as **Custom Vendor Metrics** by the user by supplying a custom link:https://github.com/payara/Payara/blob/master/appserver/payara-appserver-modules/microprofile/metrics/src/main/resources/metrics.xml[metrics.xml] file in the `domain-dir/config/` directory with the following format:
