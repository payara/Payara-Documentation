...
* With the Embedded Server API (see xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#including-the-payara-server-embedded-server-api-in-applications[Including the Payara Server Embedded Server API in Applications])
* With the EJB Embeddable API (see xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#using-the-ejb-embeddable-api-with-embedded-payara-server[Using the EJB Embeddable API with Embedded Payara Server]
...
* xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#installation-root-directory[Installation Root Directory]
* xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#instance-root-directory[Instance Root Directory]
* xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#the-domain.xml-file[The `domain.xml` File]
...
NOTE: If you have valid non-embedded Payara Server `as-install` and `domain-dir` directories, specify both in the `BootstrapProperties` and `GlassFishProperties` classes respectively as described in xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#creating-and-configuring-an-embedded-payara-server[Creating and Configuring an Embedded Payara Server].
...
To specify an existing `domain.xml` file, invoke the `setConfigFileURI` method of the `GlassFishProperties` class as described in xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#creating-and-configuring-an-embedded-payara-server[Creating and Configuring an Embedded Payara Server].
...
|References an existing xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#installation-root-directory[Installation Root Directory], also called `as-install`
a|
[source,java]
----
...
|===
|Purpose |Method |Default Value
|References an existing xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#instance-root-directory[Instance Root Directory], also called `domain-dir`
a|
[source,java]
----
...
After you create an embedded Payara Server as described in xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#creating-and-configuring-an-embedded-payara-server[Creating and Configuring an Embedded Payara Server], you can perform operations such as:

* xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#setting-the-port-of-an-embedded-payara-server-from-an-application[Setting the Port of an Embedded Payara Server From an Application]
* xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#starting-an-embedded-payara-server-from-an-application[Starting an Embedded Payara Server From an Application]
* xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#stopping-an-embedded-payara-server-from-an-application[Stopping an Embedded Payara Server From an Application]
...
* To set the port indirectly, use a `domain.xml` file that sets the port. For more information, see xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#the-domain.xml-file[The `domain.xml` File].
...
If some resources needed by an application are not under the application's directory, see xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#creating-a-scattered-archive[Creating a Scattered Archive].
...
NOTE: Ensure that your application has started an embedded Payara Server before the application attempts to run any `asadmin` commands. For more information, see xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#running-an-embedded-payara-server[Running an Embedded Payara Server].
...
For another way to change log levels, see xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#changing-log-levels-in-embedded-payara-server[Changing Log Levels in Embedded Payara Server].
...
+
See xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#setting-the-class-path[Setting the Class Path] and section 17.2 of the Jakarta Enterprise Beans specification.
...
+
See xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#default-jakarta-persistence-data-source-for-embedded-payara-server[Default Jakarta Persistence Data Source for Embedded Payara Server].
...
.  To change the xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#instance-root-directory[Instance Root Directory], set the `org.glassfish.ejb.embedded.glassfish.instance.root` system property value by using the `createEJBContainer(Map<?, ?> properties)` method.
+
...
|The xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#installation-root-directory[Installation Root Directory].
...
* `java.io.tmp` system property value
* `as-install/domains/domain1` if a non-embedded installation is referenced
|The xref:Technical Documentation/embedded-server-guide:embedded-server-guide.adoc#instance-root-directory[Instance Root Directory].
...
For more information about Payara Server logging, see xref:Technical Documentation/Payara Server Documentation/General Administration/logging.adoc#administering-the-logging-service[Administering the Logging Service] in the Payara Server General Administration section.
