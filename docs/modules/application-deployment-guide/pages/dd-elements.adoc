...
If the `persistence-type` attribute of the parent xref:dd-elements.adoc#session-manager[`session-manager`] element is `file`, the session manager passivates sessions to the persistent store when this maximum is reached.
...
+
Applicable only if the `persistence-type` attribute of the parent xref:dd-elements.adoc#session-manager[`session-manager`] element is `hazelcast`.
...
xref:dd-elements.adoc#glassfish-application[`glassfish-application`] (`payara-application.xml`),
xref:dd-elements.adoc#ejb[`ejb`] (`glassfish-ejb-jar.xml`)
