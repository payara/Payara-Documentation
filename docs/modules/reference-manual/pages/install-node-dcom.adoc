...
NOTE: To install \{product---name} software on hosts that use secure shell (SSH), use thexref:install-node-ssh.adoc#install-node-ssh-1[`install-node-ssh`(1)] subcommand.
