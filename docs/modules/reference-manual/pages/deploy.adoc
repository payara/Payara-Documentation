...
`--runtimealtdd`::
  ...
  as `glassfish-web.xml` or `payara-web.xml`. Applies to \{product---name} deployment descriptors only (`glassfish-*.xml`/`payara-*.xml`); the name of the alternate
...
* link:../application-deployment-guide/toc.html#GSDPG[Payara Server Open Source Edition Application Deployment Guide]
