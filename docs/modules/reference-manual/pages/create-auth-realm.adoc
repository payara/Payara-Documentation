...
`--property`::
  ...
  * You can specify the following properties for `CertificateRealm`: +
  `LoginModule`;;
    ...
    information, see "xref:security-guide:system-security.adoc#custom-authentication-of-client-certificate-in-ssl-mutual-authentication[Custom Authentication of Client Certificate in SSL Mutual Authentication]" in Payara Server Open
