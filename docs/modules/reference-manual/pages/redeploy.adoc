...
-`-runtimealtdd`::
  Deploys the application using a Payara Server runtime deployment descriptor that resides outside of the application archive. Specify an
  ...
  as `glassfish-web.xml` or `payara-web.xml`. Applies to Payara Server deployment descriptors only (`glassfish-*.xml`/`payara-*.xml`); the name of the alternate
