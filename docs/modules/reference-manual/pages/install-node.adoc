...
NOTE: This subcommand is equivalent to the xref:install-node-ssh.adoc#install-node-ssh-1[`install-node-ssh`(1)]
...
xref:install-node-dcom.adoc#install-node-dcom[`install-node-dcom`(1)] subcommand.
