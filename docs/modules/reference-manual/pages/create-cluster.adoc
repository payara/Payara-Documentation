...
* xref:create-instance.adoc#create-instance-1[`create-instance`(1)]
...
* Deploying applications by using the xref:deploy.adoc#deploy-1[`deploy`(1)] subcommand
* Creating resources by using subcommands such as xref:create-jdbc-resource.adoc#create-jdbc-resource-1[`create-jdbc-resource`(1)]
* Creating references to applications that are already deployed in other targets by using the xref:create-application-ref.adoc#create-application-ref-1[`create-application-ref`(1)] subcommand
* Creating references to resources that are already created in other targets by using the xref:create-resource-ref.adoc#create-resource-ref-1[`create-resource-ref`(1)] subcommand
...
`--systemproperties`::
  ...
  `JAVA_DEBUGGER_PORT`;;
    ...
    (http://www.oracle.com/technetwork/java/javase/tech/jpda-141715.adoc) debugger. Valid values are 1-65535. On UNIX, creating sockets that listen on ports 1-1024 requires superuser privileges.
  `JMS_PROVIDER_PORT`;;
...
`--properties`::
  ...
  `GMS_DISCOVERY_URI_LIST`;;
    The locations of \{product---name} instances in the cluster to use for discovering the cluster. This property is required only if the Group Management Service (GMS) is not using multicast for broadcasting messages. +
    Valid values for this property are as follows: +
    * A comma-separated list of uniform resource identifiers (URIs). Each URI must locate a \{product---name} instance or the DAS. This format is required if multiple \{product---name} instances are running on the same host. +
    ...
    ** port is the port number of the port on which the instance listens for messages from GMS. The system property
    ...
    For information about how to set this system property for an instance, see "xref:docs:ha-administration-guide:clusters.adoc#discovering-a-cluster-when-multicast-transport-is-unavailable[Discovering a Cluster When Multicast Transport
    Is Unavailable]" in Payara Server Open Source Edition High Availability Administration Guide.
...
For more information about escape characters in options for the `asadmin` utility, see the xref:asadmin.adoc#asadmin-1m[`asadmin`(1M)] help page.
...
* xref:create-application-ref.adoc#create-application-ref-1[`create-application-ref`(1)],
* xref:create-instance.adoc#create-instance-1[`create-instance`(1)],
* xref:create-jdbc-resource.adoc#create-jdbc-resource-1[`create-jdbc-resource`(1)],
* xref:create-local-instance.adoc#create-local-instance-1[`create-local-instance`(1)],
...
* xref:delete-instance.adoc#delete-instance-1[`delete-instance`(1)],
* xref:delete-local-instance.adoc#delete-local-instance-1[`delete-local-instance`(1)],
* xref:deploy.adoc#deploy-1[`deploy`(1)],
* xref:list-clusters.adoc#list-clusters-1[`list-clusters`(1)],
* xref:start-cluster.adoc#start-cluster-1[`start-cluster`(1)],
* xref:stop-cluster.adoc#stop-cluster-1[`stop-cluster`(1)],
* xref:validate-multicast.adoc#validate-multicast-1[`validate-multicast`(1)]
* "xref:../ha-administration-guide/clusters.adoc#GSHAG00373[Discovering a Cluster When Multicast Transport Is
Unavailable]" in Payara Server Open Source Edition High Availability Administration Guide
