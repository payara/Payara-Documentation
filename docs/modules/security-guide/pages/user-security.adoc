...
Custom realm::
  ...
  "xref:docs:application-development-guide:securing-apps.adoc#creating-a-custom-realm[Creating a Custom Realm]" in the Payara Server Application Development Guide.
...
+
See "xref:administration-guide:jdbc.adoc#to-create-a-jdbc-connection-pool[To Create a JDBC Connection Pool]" in the Payara Server Administration Guide.

. Create a JDBC resource for the database.
+
See "xref:administration-guide:jdbc.adoc#to-create-a-jdbc-resource[To Create a JDBC Resource]" in the Payara Server Administration Guide.
...
+
For more information about how to specify a security realm, see "xref:docs:application-development-guide:securing-apps.adoc#how-to-configure-a-realm[How to Configure a Realm]" in Payara Server Application Development Guide.
...
NOTE: If secure administration is enabled as described in xref:administrative-security.adoc#running-secure-admin[Running Secure Admin], you cannot create an administrative user using a blank password.

Creating a new `file` realm user is a dynamic event and does not require server restart.

. Ensure that the server is running.
. Review the current groups of the security realm by using the xref:reference-manual:list-file-groups.adoc[`list-file-groups`] subcommand.
...
The `asadmin` `--passwordfile` option specifies the name of a file that contains the password entries in a specific format. The entry for a password must have the `AS_ADMIN_` prefix followed by the password name in uppercase letters, an equals sign, and the password. See the xref:reference-manual:asadmin.adoc[`asadmin`] documentation for more information.
...
NOTE: If secure administration is enabled as described in xref:administrative-security.adoc[Running Secure Admin], you cannot update an administrative user to have a blank password.
