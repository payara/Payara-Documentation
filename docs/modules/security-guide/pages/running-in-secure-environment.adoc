...
This chapter assumes that you are familiar with security features such as authentication, authorization and certificates. If you are not, see xref:system-security.adoc#administering-system-security[Administering System Security] first.
...
By default, Payara Server logging messages are recorded in the server log, and you can set the file rotation limit, as described in xref:reference-manual:rotate-log.adoc[`rotate-log`]
...
However, when third parties use Payara Server and untrusted classes are being run, the Java security manager may be useful. See "xref:application-development-guide:securing-apps.adoc#enabling-and-disabling-the-security-manager[Enabling and Disabling the Security Manager]" in the Application Development Guide.
