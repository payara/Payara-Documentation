...
A server instance is a Payara Server running in a single Java Virtual Machine (JVM) process. The supported platforms for Payara Server are described in the following article: xref:docs:ROOT:General Info/Supported Platforms.adoc[Supported Platforms].
