...
== Behaviour Changes
The default (undefined) value of the `fish.payara.ready-after-applications` system property will change from `false` to `true` starting from 5.91.0, aligning Payara 6 with Community and Payara 7.
...
This may change the timing of post-boot scripts and transaction recovery. To maintain the current behaviour after the change, set `fish.payara.ready-after-applications=false`.
