...
== Behaviour Changes
The default (undefined) value of the `fish.payara.ready-after-applications` system property has changed from `false` to `true`, aligning Payara 6 with Community and Payara 7.
The server now marks itself as ready after post-boot configuration is applied and previously deployed applications are loaded, rather than after the `server_startup` event.
This may change the timing of post-boot scripts and transaction recovery.
To restore the previous behaviour, set `fish.payara.ready-after-applications=false`.
