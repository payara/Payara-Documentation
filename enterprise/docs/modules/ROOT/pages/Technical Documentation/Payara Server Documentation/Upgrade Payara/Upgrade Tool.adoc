...
NOTE: The upgrade tool needs to be updated to the latest version if trying to update Payara 5.40 or below. Download the latest version of the upgrade tool from https://nexus.payara.fish/#browse/browse:payara-enterprise-downloadable-artifacts:fish%2Fpayara%2Fextras%2Fpayara-upgrade-tool[Payara Nexus] and replace the existing upgrade tool JAR in the `payara5/glassfish/lib/asadmin` directory.
