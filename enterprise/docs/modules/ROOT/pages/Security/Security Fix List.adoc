...
|https://www.cve.org/CVERecord?idx=CVE-2026-12986[CVE-2026-12986] | 7.9 | FIXED | The admin console leaked the admin REST token to attacker-controlled
hosts via an unvalidated restUrl parameter, enabling
unauthenticated remote code execution. | 5.88.0, 4.1.2.191.56 | Fixed by replacing user-supplied restUrl with a trusted server-side session
attribute.

|https://nvd.nist.gov/vuln/detail/cve-2025-52911[CVE-2025-52911] | - | FIXED | RFC 9110 states that a recipient MUST parse and ignore a reasonable number of empty list elements: enough to handle common mistakes by senders that merge values, but not so much that they could be used as a denial-of-service mechanism.  | 5.78.0 | Fixed by adding extra header validation

|https://nvd.nist.gov/vuln/detail/CVE-2026-24457[CVE-2026-24457] | 9.1 | FIXED | An unsafe parsing of OpenMQ's configuration, allows a remote attacker to read arbitrary files from a MQ Broker's server. A full exploitation could read unauthorized files of the OpenMQ’s host OS. In some scenarios RCE could be achieved. | 5.87.0, 4.1.2.191.55 | Fixed by patching OpenMQ.

|https://nvd.nist.gov/vuln/detail/CVE-2020-27511[CVE-2020-27511] | 7.5 | FIXED | An issue discovered in the stripTags and unescapeHTML components in Prototype 1.7.3 allows an attacker to cause a Regular Expression Denial of Service (ReDOS) through stripping crafted HTML tags. | 5.83.0, 4.1.2.191.54 | Fixed by upgrading the prototype library.

|https://nvd.nist.gov/vuln/detail/CVE-2025-14340[CVE-2025-14340] | 9.3 | FIXED | A Cross-Site Scripting vulnerability in Payara’s Administration Rest Interface, allows execution of attacker-controlled JavaScript leading to admin account takeover. | 5.83.0, 4.1.2.191.54 | Fixed by encoding the error message in Payara Server.

|https://nvd.nist.gov/vuln/detail/CVE-2020-36843[CVE-2020-36843] | 4.3 | FIXED | The implementation of EdDSA in EdDSA-Java (aka ed25519-java) through 0.3.0 exhibits signature malleability and does not satisfy the SUF-CMA (Strong Existential Unforgeability under Chosen Message Attacks) property. | 5.82.0 | Fixed by upgrading dependency for eddsa used by Trilead SSH in Payara Server.
